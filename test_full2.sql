...
abc_selected as (
  select ranked.rotation_category as recommendation_group,
    ranked.product_id, ranked.sku, ranked.barcode, ranked.description, ranked.unit,
    ranked.cost, ranked.system_stock, ranked.inventory_value, ranked.rotation_category, ranked.period_month
  from (
    select cand.product_id, cand.sku, cand.barcode, cand.description, cand.unit,
      cand.cost, cand.system_stock, cand.inventory_value, cand.rotation_category, cand.period_month,
      row_number() over (order by case cand.rotation_category when 'A' then 0 when 'B' then 1 when 'C' then 2 end, cand.inventory_value desc, cand.sku) as rn
    from candidates cand where cand.rotation_category in ('A','B','C')
  ) ranked where ranked.rn <= 15
),
value_selected as (
  select 'VALORIZADO'::text as recommendation_group,
    ranked.product_id, ranked.sku, ranked.barcode, ranked.description, ranked.unit,
    ranked.cost, ranked.system_stock, ranked.inventory_value, ranked.rotation_category, ranked.period_month
  from (
    select cand.product_id, cand.sku, cand.barcode, cand.description, cand.unit,
...
select recommendation_group, sku, rotation_category, inventory_value
from (
  select 0 as grp, recommendation_group, product_id, sku, barcode, description, unit, cost, system_stock, inventory_value, rotation_category, period_month from abc_selected
  union all
  select 1 as grp, recommendation_group, product_id, sku, barcode, description, unit, cost, system_stock, inventory_value, rotation_category, period_month from value_selected
) combined
order by grp, inventory_value desc, sku
limit 30;
