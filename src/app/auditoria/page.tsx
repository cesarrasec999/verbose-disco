...
type ResumenRow = {
    ...
    notes: string[]; // Observaciones de los conteos
};
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        if (user.role === "Operario") {
            ...
            // Restaurar datos del validador si había una tienda/fecha guardada
            // NOTA: Para el admin, el tab activo puede ser "admin" pero igual necesitamos
            // cargar los datos del validador para que el Resumen por código funcione correctamente.
            const savedValStore = sessionStorage.getItem("cyclic_val_store");
            const savedValDate  = sessionStorage.getItem("cyclic_val_date");
            if (savedValStore && savedValStore !== ALL_STORES_VALUE && savedValDate) {
    ...
    useEffect(() => {
        ...
        // Cuando se navega a registros o resumen con tienda ya seleccionada, recargar datos
        if ((valTab === "registros" || valTab === "resumen") && valStoreId && valStoreId !== ALL_STORES_VALUE) {
    ...
    const resumenPorCodigo = useMemo((): ResumenRow[] => {
        ...
        for (const c of counts) {
            ...
            if (entry) {
                ...
                // Recolectar notas/observaciones no vacías
                if (c.note && c.note.trim()) {
                    entry.notes.push(c.note.trim());
                }
            }
        }
                                                ...
                                                <thead className="bg-slate-100 sticky top-0">
                                                    <tr>
                                                        {([
                                                            ...
                                                            { col: "obs",     label: "Observaciones",   align: "left"   as const, amber: false },
                                                ...
                                                <tbody>
                                                    {filteredResumen
                                                        ...
                                                        <tr key={r.product_id} className={
                                                            ...
                                                            <td className="p-2 border text-left text-xs text-slate-600 max-w-[200px]">
                                                                {r.notes.length > 0
                                                                    ? r.notes.map((n, i) => (
