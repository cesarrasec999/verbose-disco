...
export default function AuditoriaModule({ mainTab, registerTab: registerTabProp }: { mainTab: MainTab; registerTab?: RegisterTab }) {
  ...
  // Estos efectos antes cambiaban mainTab/registerTab en memoria (setMainTab/
  // setRegisterTab); ahora que cada pestaña es una ruta real, la misma regla
  // de negocio se traduce a una navegacion (router.replace) hacia la ruta
  // correcta en vez de un cambio de estado local.
  useEffect(() => {
    if (!user) return;
    if (isMobileAccess) {
  ...
  async function openSession(row: AuditSession) {
    ...
    // La sesion elegida se persiste en sessionStorage (no en el estado de
    // este componente, que esta a punto de desmontarse) para que la ruta de
    // destino la recupere en su propio bootstrap (loadSavedSession), igual
    ...
    <main className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <div className="mx-auto max-w-7xl px-3 py-4 md:px-5">
        <div className={`mb-4 hidden gap-2 rounded-2xl border bg-white p-1.5 shadow-sm md:grid ${canViewAuditSummary ? "grid-cols-3" : "grid-cols-2"}`}>
          ...
          {canViewAuditSummary && (
            <Link href="/auditoria/resumen-admin" className={tabClass(visibleMainTab === "adminSummary")}><BarChart3 size={16} /> Resumen</Link>
          )}
        </div>
                ...
                {session && (
                  <div className="mt-4 space-y-2 rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      ...
                      <button onClick={() => { clearSelectedSession(); setMessage("Sesion liberada. Selecciona tienda y crea otra auditoria."); }} className="rounded-xl border border-blue-200 bg-white px-4 py-3 font-bold text-blue-800">Nueva auditoria</button>
                    </div>
                  </div>
                )}
                {!session ? (
                      ...
                      <tbody>
                        {filteredSummaryRows.map(r => (
                          <tr key={r.item.id} className="border-b hover:bg-slate-50">
                            ...
                            <td className="p-2">
                              <textarea
                                value={itemObservationDrafts[r.item.id] ?? r.item.observation ?? ""}
