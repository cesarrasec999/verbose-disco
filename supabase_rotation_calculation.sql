...
BEGIN
  ...
  WITH stores_gpc AS (
    ...
    SELECT
      ...
      -- real de tienda del ERP (confirmado: GPC002 LIM-SUMINISTRO es la
      -- tienda real 4, no 2; GPC003 LIM-GRUPO es la 5, etc). Se usa el numero
      -- de la etiqueta con una correccion fija para esos 4 casos conocidos.
      -- No se usa erp_store_no: ese campo lo repuebla un sync externo y ya
      -- volvio a quedar mal sin aviso una vez.
