...
select recommendation_group, product_id, sku, barcode, description, unit,
  cost, system_stock, inventory_value, rotation_category, period_month
from abc_selected
union all
select recommendation_group, product_id, sku, barcode, description, unit,
  cost, system_stock, inventory_value, rotation_category, period_month
from value_selected
order by case recommendation_group when 'A' then 0 when 'B' then 1 when 'C' then 2 else 3 end, inventory_value desc, sku;
