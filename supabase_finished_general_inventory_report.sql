-- Reporte de inventarios generales finalizados por rango de fecha.
-- Ejecutar en Supabase SQL Editor. Es solo lectura: no modifica conteos ni sesiones.

create index if not exists idx_gi_sessions_finished_report
  on public.general_inventory_sessions(status, finished_at);

create index if not exists idx_gi_stock_snapshot_session_product_report
  on public.general_inventory_stock_snapshot(session_id, product_id);

create index if not exists idx_gi_counts_session_product_report
  on public.general_inventory_counts(session_id, product_id);

create index if not exists idx_gi_recount_items_session_product_status_report
  on public.general_inventory_recount_items(session_id, product_id, status);

create index if not exists idx_gi_recount_counts_session_item_report
  on public.general_inventory_recount_counts(session_id, recount_item_id);

create index if not exists idx_gi_validation_items_session_product_status_report
  on public.general_inventory_validation_items(session_id, product_id, status);

create index if not exists idx_gi_validation_counts_session_item_report
  on public.general_inventory_validation_counts(session_id, validation_item_id);

create index if not exists idx_gi_non_inventory_session_sku_report
  on public.general_inventory_non_inventory_products(session_id, sku);

create or replace function public.get_finished_general_inventory_report(
  p_date_from date,
  p_date_to date
)
returns table (
