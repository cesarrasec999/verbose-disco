-- Algunos controles antiguos no llenaron full_location, pero sí sus cuatro
-- campos estructurados. Se usa ese dato real para generar, por ejemplo,
-- ALMACEN-G-RACK-[5], sin inventar ubicaciones.
...
begin
  ...
  if exists (
    ...
    where s.id = new.store_id
      and (upper(coalesce(s.name, '')) like '%CD-GPC%' or upper(coalesce(s.name, '')) like '%IMPORT%')
  ) then
    return new;
  end if;
...
with latest_control as (
  ...
  from (
    select gis.store_id, gil.session_id,
      row_number() over (partition by gis.store_id order by max(gil.created_at) desc, max(gis.scheduled_date) desc, gil.session_id desc) as rn
    from public.general_inventory_locations gil
    join public.general_inventory_sessions gis on gis.id = gil.session_id
    where coalesce(
      case when nullif(btrim(gil.full_location), '') is not null and btrim(gil.full_location) !~ '^\d+$' then btrim(gil.full_location) end,
  ...
  select p.id as source_id, p.store_id, p.product_id, tm.full_location, p.last_source, p.last_seen_at,
    p.cyclic_registered, p.audit_registered, p.general_inventory_registered, p.stored_quantity,
    p.updated_by, p.updated_at as source_updated_at,
    (
      select p2.id from public.product_locations p2
      where p2.store_id = p.store_id and p2.product_id = p.product_id and p2.location = tm.full_location and p2.id <> p.id
      order by p2.is_active desc, p2.updated_at desc nulls last limit 1
    ) as target_id
  from public.product_locations p
  join public.stores s on s.id = p.store_id
  join ticket_map tm on tm.store_id = p.store_id and tm.ticket_key = regexp_replace(btrim(p.location), '^0+(?=\d)', '')
  where p.is_active = true and btrim(p.location) ~ '^\d+$'
    and upper(coalesce(s.name, '')) not like '%CD-GPC%'
    and upper(coalesce(s.name, '')) not like '%IMPORT%'
), convertible as (
  select conversion_base.*, row_number() over (partition by store_id, product_id, full_location order by source_updated_at desc nulls last, source_id) as conversion_rank
  from conversion_base
)
select * into temporary table ticket_location_conversion_fallback from convertible;

update public.product_locations p
set location = c.full_location, updated_at = now()
from ticket_location_conversion_fallback c
where c.target_id is null and c.conversion_rank = 1 and p.id = c.source_id;

update public.product_locations p
set is_active = true, last_source = c.last_source, last_seen_at = c.last_seen_at,
    ...
    stored_quantity = coalesce(c.stored_quantity, p.stored_quantity), updated_by = c.updated_by, updated_at = now()
from ticket_location_conversion_fallback c
where c.target_id is not null and c.conversion_rank = 1 and p.id = c.target_id;

update public.product_locations p
set is_active = false, updated_at = now()
from ticket_location_conversion_fallback c
where (c.target_id is not null or c.conversion_rank > 1) and p.id = c.source_id;
