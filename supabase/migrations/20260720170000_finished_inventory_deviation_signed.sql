-- Invierte el signo de deviation_over_sales_pct.
-- Diferencia positiva, sobrante, ahora da desviacion negativa.
-- Diferencia negativa, faltante, ahora da desviacion positiva.
-- Asi un sobrante en una tienda compensa un faltante en otra al
-- calcular el indicador general ponderado contra la venta total.
-- No hace falta DROP porque el tipo de retorno no cambia, solo el
-- calculo dentro del CASE final.
...
select
  ...
  case
    when coalesce(sbs.sales_in_period, 0) > 0
      then round((-coalesce(ss.net_value_diff, 0) / sbs.sales_in_period) * 100, 2)
    else null
